...
if __name__=="__main__":
    ...
    # dataloader
    transform2Tensor = transforms.Compose([transforms.RandomHorizontalFlip(0.5), transforms.Resize((image_size, image_size)), transforms.ToTensor(),
                                           transforms.Normalize(mean=[normal_mean]*channels, std=[normal_std]*channels)])
    train_datasets = datasets.CelebA("datasets/data", transform=transform2Tensor, split="train", download=False)
    train_dataloader = torch.utils.data.DataLoader(train_datasets, batch_size=batch_size, shuffle=True)
    print(f"Dataloader Done!")

    unet = UNetModel(input_channels=channels, channel_mul_layer=channel_mul, num_head=1).to(device=device)
